<?php
/**
 * JUMultiThumb - plugin for resize image
 *
 * @version 	7.x
...
class plgContentJUMULTITHUMB_Gallery extends JPlugin
{
    ...
    public function onContentPrepare($context, &$article, &$params, $limitstart)
	{
        ...
        if(isset($article->text)) $article->text = @$this->GalleryReplace($article->text, $article);

        if(isset($article->fulltext)) $article->fulltext = @$this->GalleryReplace($article->fulltext, $article);
    }
